...
- [ ] 含密钥的本地配置层已被 gitignore（验证：`git check-ignore` 命中 `.bluecode/settings.local.yaml`）；对话区与任何输出均不出现 `api_key`（验证：通读输出）。(AC17)
...
- [ ] 场景 4（永久放行 + 文件产物）：对某调用选「永久」→ (a) 用 `cat`/`Grep` 确认本地层配置文件出现该精确 allow 条目；(b) 重启 bluecode 后同调用不再弹窗直接执行。(AC10/F8)
