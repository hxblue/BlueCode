...
需要的是**空间维度**的隔离:同一仓库同时挂多个工作目录、共享版本库、各自一个分支。这就是 Git Worktree (Git 2.5+) 的能力。本章在 bluecode 中封装一层 Worktree 管理逻辑,把这块拼图补给 SubAgent,让后台 / 并行场景安全可用。

bluecode 现有相关基础设施:
- ch13 SubAgent 已支持 frontmatter (`dev.bluecode.subagent.Parser`),解析 `name/description/tools/disallowedTools/model/maxTurns/permissionMode/background` 等字段
- ch13 `agent.AgentTool#execute` 已是子 Agent 启动入口,本章在此处插桩 isolation 分支
- ch08 文件读缓存以绝对路径作为 key
- ch10 `dev.bluecode.command` 已有 slash 命令注册系统
- `.gitignore` 已忽略 `.bluecode/sessions/` 等子目录,本章扩展把 `.bluecode/worktrees/` 也忽略
...
- **G1**: 提供 `WorktreeManager` 封装 Worktree 完整生命周期——创建、快速恢复、进入、退出、删除;并发场景下用单一 `ReentrantLock` 保护内部 `active` 映射
- **G2**: 名字 (slug) 严格安全校验——限字符集 `[a-zA-Z0-9._-]`、总长度上限 64、显式拒绝 `.` 和 `..` 段名、允许 `/` 做嵌套分隔;防 LLM 输入触发路径遍历
- **G3**: Worktree 目录统一落在仓库内不被追踪的位置 `.bluecode/worktrees/<flatSlug>/`,分支名前缀 `worktree-<flatSlug>`,嵌套 slug 的 `/` 替换为 `+` 避免 Git D/F 冲突
- **G4**: 创建后做四类环境初始化——A 复制本地配置 (`.bluecode/config.yaml` / `.bluecode/settings.local.yaml`)、B 配置子目录的 git hooks (`core.hooksPath` 不自动继承)、C 软链 `node_modules` / `.venv` / `vendor` 等大目录、D 按项目根 `.worktreeinclude` 复制被忽略但运行需要的文件;均为 best-effort,失败只警告不中断创建
...
- **G11**: `WorktreeSession` 持久化到 `.bluecode/worktree_session.json`,bluecode 启动时读取并校验目录仍存在;退出时写空 JSON `null` 而不是删文件,确保下次启动不误恢复
...
- **F1**: `WorktreeSlug.validate(name)` 校验规则——
  - name 非空,总长度 ≤ 64
...
- **F2**: `Worktree` record 记录单个 Worktree 的元信息——`name`(原始 slug)、`path`(绝对路径)、`branch`(`worktree-<flatSlug>`)、`basedOn`(创建时的 base 引用,如 `HEAD` 或具体 commit)、`headCommit`(创建时的 commit SHA)、`created`(`Instant`)、`manual`(boolean,是否用户手动创建,影响 autoCleanup 跳过判断)
...
- **F4**: `WorktreeManager` 内部字段——`repoRoot`(绝对路径)、`worktreeDir`(`<repoRoot>/.bluecode/worktrees`)、`sessionFile`(`<repoRoot>/.bluecode/worktree_session.json`)、`ReentrantLock lock`、`Map<String, Worktree> active`、`WorktreeSession currentSession`
- **F5**: `new WorktreeManager(Path repoRoot)` 构造时——
  - 校验 `repoRoot` 是 git 仓库根目录(`git rev-parse --show-toplevel` 输出与之等);失败抛 `IOException`,bluecode 启动允许降级到「Worktree 功能未启用」
...
- **F6**: `manager.create(name, baseRef, manual)`——
  ...
  - 8. 加入 `active`,返回
- **F7**: 创建后设置 A——复制本地配置文件,从 `<repoRoot>/.bluecode/config.yaml` 与 `<repoRoot>/.bluecode/settings.local.yaml` 复制到 Worktree 同位置(目标已存在跳过,文件不存在跳过)
...
- **F16**: 在 `dev.bluecode.tool` 包定义 ctx key 与帮助函数(`ToolContext` 不可变上下文对象封装)——
...
- **F20**: 扩展 `subagent.Definition` 增加 `String isolation` 字段;`Parser` 解析 frontmatter `isolation:` 字段,合法值 `""` / `"worktree"`,非法值 stderr 警告后回落到 `""`
