...
## 实现完整性
- [ ] 多轮自动连环：需要连续两步工具的任务，Agent 无需中途催促即自动多轮执行工具直到给出最终答复（验证：`./gradlew -q exec:java -Dexec.mainClass=com.bluecode.BlueCode` 或 `java -jar build/libs/bluecode.jar` 跑「读 A 文件 → 据内容新建 B 文件」，观察 read_file 与 write_file 跨多轮依次出现、最终答复）。(AC1/F1)
...
- [ ] 界面不阻塞：多轮循环与工具执行（含并发批）期间 spinner / 轮次 / 计时持续刷新（验证：跑含稍慢 bash 的任务，观察界面不冻结；virtual thread 跑工具与 SDK 流，UI 线程通过 `program.send()` 更新）。(N2)
...
- [ ] 取消无泄漏：取消后无挂起 virtual thread / 无未关闭 `BlockingQueue`（验证：`./gradlew -q test -Dtest=AgentTest` 含取消用例（场景 E）通过；端到端反复触发取消后继续对话多次，进程内存 / 句柄稳定不增长，可用 `jcmd <pid> Thread.print | grep "Virtual"` 抽查）。(N5/N6)
- [ ] 系统提示体现 Agent 循环：问「你能做什么」答复体现可多步使用工具完成任务（验证：发一条询问观察答复）。(F3)

## 编译与测试
- [ ] `./gradlew shadowJar` 无错误（fat jar 可启动）。
- [ ] `./gradlew -q test` 通过（`ConfigLoaderTest`、`ConversationTest`、`AgentTest`、`tool` 相关用例）。
- [ ] `./gradlew -q test -Dtest='com.bluecode.agent.*,com.bluecode.tool.*'` 无失败（覆盖 N6 关键包，含并发场景 D / 取消场景 E）。
- [ ] `./gradlew compileJava` (代码风格由 IDE 保证) 通过（google-java-format 合规）。(N8)
